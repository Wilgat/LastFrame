...
│   └── FirstFrame-spec.md     # Project specifications: supported formats (MP4 only), OpenCV dependencies, Cython directives (e.g., language_level=3)
...
└── src/
    └── FirstFrame/
        ...
        ├── __init__.py       # Package initialization: exposes ChronicleLogger (recommend rename to FirstFrameExtractor for alignment) and version
...
- **Cython-Specific**: Convert `cli.py` to `.pyx` for compiling performance bottlenecks like `VideoCapture` loops; `build.sh` automates `cythonize -i src/FirstFrame/cli.pyx` and `python -m build`   .

## Project Name Conversion Rules

- **Package Name**: `FirstFrame` (CamelCase for module/directory, per PEP 8 and CyMaster norms).
- **File Naming**: Snake_case for Python files (e.g., `cli.py`); descriptive lowercase for docs (e.g., `FirstFrame-spec.md`).
...
- **Temporary/Intermediates**: None explicit; Cython builds produce `.c`, `.so`, or executables prefixed with `FirstFrame_` (e.g., `FirstFrame_cli.cpython-312-x86_64-linux-gnu.so`) in `build/bin/` subdirs like `amd64-glibc/` or `armv7l-glibc/`   .
...
2. **Run Directly**:
   ```
   python -m src.FirstFrame
   ```
   Or post-build: `./build/bin/amd64-glibc/FirstFrame` (after `build.sh`).
...
4. **Cython Build** (via `build.sh`, POSIX-compliant):
   ...
   # ===== BEGIN NEW CODE =====
   # NEW: cythonize -i src/FirstFrame/cli.pyx
   # NEW: python -m build --wheel
   # NEW: # Outputs to build/bin/amd64-glibc/FirstFrame executable
   # ===== END NEW CODE =====
   chmod +x build.sh
   ./build.sh
...
5. **Package Install**: Future PyPI: `pip3 install FirstFrame`; runs as `python -m FirstFrame` .
